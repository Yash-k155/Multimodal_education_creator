...
export default function Generator({ content, setContent, isLoading, setIsLoading, error, setError }: GeneratorProps) {
  const handleGenerate = async (topic: string, isSearch = false) => {
    setIsLoading(true);
    setError(null);
    setContent(null);
    ...
    try {
      if (isSearch) {
        const res = await axios.get(`/search`, { params: { query: topic } });
        ...
        const res = await axios.post(`/generate-content`, { topic });
        setContent(res.data);
      }
    } catch (err: any) {
    ...
    <div className="space-y-8">
      ...
      {content?.searchResults && (
        <div className="card max-w-2xl mx-auto">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Search Results for "{content.query}"</h2>
          {content.searchResults.map((r: any, i: number) => (
      ...
      {content && !content.searchResults && (
        <>
          <div className="card max-w-full bg-gradient-to-r from-blue-600 to-indigo-700 text-white" style={{ backgroundColor: 'initial' }}>

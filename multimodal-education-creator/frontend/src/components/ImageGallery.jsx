...
function ImageCard({ url, label, index }) {
  ...
  return (
    <div className="card p-0 overflow-hidden">
      ...
      <div className="px-4 py-3">
        ...
        <p className="text-sm text-gray-700 mt-0.5 line-clamp-2">{label}</p>
      </div>
    </div>
  );
}

export default function ImageGallery({ imageUrls, flashcards }) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {imageUrls.map((url, i) => (
          <ImageCard
            key={i}
            url={url}
            label={flashcards?.[i]?.question || `Flashcard ${i + 1}`}
            index={i}
          />
        ))}
      </div>
